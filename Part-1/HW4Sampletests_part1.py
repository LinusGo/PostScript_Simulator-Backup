...
class HW4Sampletests_part1(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        copy()
        self.assertTrue(
            opPop() == 4 and opPop() == 3 and opPop() == 1 and opPop() == 4 and opPop() == 3 and opPop() == 1 and opPop() == True)
    ...
    def test_getinterval1(self):
        # (ILOVECPTS355) 8 3 getinterval
        opPush('ILOVECPTS355')
        opPush(0)
        opPush(4)
        getinterval()
        self.assertEqual(opPop(), '(LOVE)')
        self.assertTrue(len(opstack) == 0)
        # getinterval will not push back the string onto the opstack
    ...
    def test_putinterval4(self):
        # (ILIKECPTS355) dup 0 (LOVE) putinterval
        opPush('(ILIKECPTS355)')
        dup()  # duplicating the string reference
        opPush(1)
        opPush('(LOVE)')
        putinterval()  # putinterval will not push back the changed string onto the opstack
        self.assertEqual(opPop(), '(ILOVECPTS355)')  # we pop the string reference we copied with "dup"
        self.assertTrue(len(opstack) == 0)
    ...
    def test_search3(self):
        # (ILOVECPTS355) (LOVE) search
        # push the string that will be split
        opPush('(ILOVECPTS355)')
        # push the 'seek' string
        opPush('(L)')
        search()
        self.assertTrue(opPop() == True and opPop() == '(I)' and opPop() == '(L)' and opPop() == '(OVECPTS355)')
    ...
    def test_evaluateArray1(self):
        ...
        # [1 1 1 add y x sub]
        # [(aBCd) 1 2 getinterval (BC) eq]
        self.assertEqual(evaluateArray([1, 2, 3, 4, 'x', 'y']), [1, 2, 3, 4, 5, 6])
        self.assertEqual(evaluateArray([1, 1, 1, 1, 'y', 'x', 'sub']), [1, 1, 1, 1, 1])
        self.assertEqual(evaluateArray([1, 'dup', 'y', 'x', 'add']), [1, 1, 11])

    def test_aload(self):
        # [3 5 5 True4] aload
        opPush([True, '(CptS)', 3, 5, 5, '(Fall)', 2020])
        aload()
        self.assertTrue(opPop() == [True, '(CptS)', 3, 5, 5, '(Fall)',
                                    2020] and opPop() == 2020 and opPop() == '(Fall)' and opPop() == 5 and opPop() == 5
    ...
    def test_begin(self):
        # /x 5 def 1 dict begin /y 10 def end x
        opPush(1)
        psDict()
        opPush("/x")
        opPush(5)
        ...
        self.assertEqual(lookup('x'), 5)

    def test_End(self):
        # /x 10 def 1 dict begin /x 20 def end x
        opPush(1)
        psDict()
        opPush("/x")
        opPush(10)
